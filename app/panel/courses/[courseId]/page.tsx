...
export default function TrainingPage({ params }: { params: Promise<{ courseId: string }> }) {
          ...
          if (q.type === 'multi-task' && q.subTasks) {
            q.subTasks.forEach(subTask => {
              ...
              if (answer !== undefined) {
                ...
                const result = lessonResults[subTask.id];
                subTaskResults[subTask.id] = (result === 'correct' || result === 'incorrect') ? null : (result || null);
                if (status) {
  ...
  const saveProgress = async (lessonId: string, completed: boolean) => {
    ...
    try {
      ...
      // Log dla debugowania
      console.log('✅ Zapisano postęp:', { lessonId, completed: finalCompleted, quizCompleted, hasQuiz: (lesson?.quiz?.length || 0) > 0 });
  ...
  const saveQuizResponse = async (questionId: string, answer: string | number, status: 'pending' | 'approved' = 'pending', subTaskId?: string) => {
    ...
    try {
      ...
      if (subTaskId) {
        // Dla podzadań
        const subTaskStatus: 'pending' | 'approved' | 'rejected' = (finalStatus as string) === 'approved' ? 'approved' : (finalStatus as string) === 'rejected' ? 'rejected' : 'pending';
        setQuizSubTaskResults(prev => ({ ...prev, [subTaskId]: subTaskStatus }));
      } else if (question?.type === 'open') {
        // Dla zwykłych pytań otwartych
        setResponseStatuses(prev => ({ ...prev, [questionId]: finalStatus }));
        const questionStatus: 'pending' | 'approved' | 'rejected' = (finalStatus as string) === 'approved' ? 'approved' : (finalStatus as string) === 'rejected' ? 'rejected' : 'pending';
        setQuizResults(prev => ({ ...prev, [questionId]: questionStatus }));
      }
    } catch (error) {
      console.error('Błąd zapisywania odpowiedzi:', error);
    }
  ...
  const handleCheckQuiz = async () => {
    ...
    const allCompleted = currentLesson.quiz.every(q => {
      if (q.type === 'choice') {
        ...
        const result = newResults[q.id];
        return (result as string) === 'pending' || (result as string) === 'approved';
      }
    });
