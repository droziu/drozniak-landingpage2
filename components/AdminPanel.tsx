...
export const AdminPanel: React.FC = () => {
  ...
  useEffect(() => {
    console.log('AdminPanel useEffect - user:', user?.email || 'brak użytkownika');
    if (!user) {
      console.log('AdminPanel - brak użytkownika, czekam...');
      return;
    }
    if (user.email !== 'stanislaw@drozniak.com') {
      console.log('AdminPanel - użytkownik nie jest adminem, przekierowanie do /login');
      navigate('/login');
      return;
    }
    console.log('AdminPanel - użytkownik jest adminem, ładowanie użytkowników');
    loadUsers();
  }, [user, navigate]);
  ...
  const loadUsers = async () => {
    ...
    try {
      console.log('AdminPanel - próba pobrania użytkowników jako admin:', user.email);
      
      // Pobierz wszystkich użytkowników z profiles (BEZ email - email jest w auth.users)
      const { data: profiles, error: profilesError } = await supabase
        .from('profiles')
        .select('id, full_name, company_name, first_name, last_name, phone, notes, created_at')
        .order('created_at', { ascending: false });

      console.log('AdminPanel - wyniki zapytania profiles:', { 
      ...
      // Pobierz emaile używając funkcji RPC dla każdego użytkownika
      const usersWithEmail = await Promise.all(
        (profiles || []).map(async (profile) => {
          try {
            ...
            if (emailError) {
              console.error('Błąd pobierania email dla użytkownika:', profile.id, emailError);
            }
            ...
            console.error('Błąd pobierania email dla użytkownika:', profile.id, err);
            return {
              ...profile,
              email: null,
            };
      ...
      console.log('Załadowano użytkowników:', usersWithEmail.length);
      console.log('Użytkownicy:', usersWithEmail);
      setUsers(usersWithEmail);

      // Dla każdego użytkownika oblicz postęp
      const progressDataArray: UserProgress[] = [];
      
      console.log('Rozpoczynam obliczanie postępu dla', usersWithEmail.length, 'użytkowników');
      ...
      for (const profile of usersWithEmail) {
        ...
        if (progressError) {
          console.error('Błąd pobierania postępu dla użytkownika:', profile.id, progressError);
        }
        ...
        if (responsesError) {
          console.error('Błąd pobierania odpowiedzi dla użytkownika:', profile.id, responsesError);
        }

        // Oblicz ukończone lekcje
        ...
        trainingModules.forEach(module => {
          ...
          const completedInModule = module.lessons.filter(lesson => {
            ...
            // Sprawdź czy wszystkie pytania w lekcji są ukończone
            const allQuizCompleted = lesson.quiz.every(q => {
              ...
              if (q.type === 'choice') {
                ...
                // Używamy tej samej logiki co w TrainingPage - konwersja na number
            ...
            // Lekcja jest ukończona jeśli wszystkie quizy są ukończone (tak jak w TrainingPage)
        ...
        console.log('Dodano postęp dla użytkownika:', userProgressItem.userName, userProgressItem.percentage + '%');
        progressDataArray.push(userProgressItem);
      }

      console.log('Obliczono postęp dla:', progressDataArray.length, 'użytkowników');
      console.log('Postęp:', progressDataArray);
  ...
  const handleQuickApprove = async (responseId: string, e: React.MouseEvent) => {
    e.stopPropagation(); // Zapobiegaj otwieraniu szczegółów
  ...
  console.log('AdminPanel render - user:', user?.email || 'brak użytkownika', 'authLoading:', authLoading, 'loading:', loading);
  ...
  if (!user || user.email !== 'stanislaw@drozniak.com') {
    console.log('AdminPanel - brak dostępu, zwracanie null');
    return null;
  }
      ...
      <div className="container mx-auto px-6 py-8 max-w-7xl">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400">Zarządzanie kursantami i weryfikacja odpowiedzi</p>
          </div>
          <button
            onClick={async () => {
              await signOut();
              ...
              {userProgress.length === 0 ? (
                ...
                userProgress.map((progress) => (
                  <div key={progress.userId}>
                    {/* Zwarty widok - tylko email i procent */}
                    <div
                    ...
                    {/* Rozwinięty panel z pełnymi informacjami */}
                    {selectedUser === progress.userId && (
                      ...
                      {selectedUserResponses.length === 0 ? (
                        ...
                        selectedUserResponses.map((response) => {
                          ...
                          return (
                            ...
                            >
                              {/* Szybkie zatwierdzenie - checkmark */}
