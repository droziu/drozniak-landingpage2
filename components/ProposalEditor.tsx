...
export const ProposalEditor: React.FC = () => {
    ...
    const initializeEditor = async () => {
      if (user) {
        try {
          ...
          // Zawsze ustaw loading na false, nawet jeśli jest błąd
          setLoading(false);
          // Dla nowej oferty, nie pokazuj błędu - po prostu pozwól użytkownikowi pracować
          if (!isNew) {
            setAlertModal({
              isOpen: true,
              title: 'Błąd',
        ...
        // Jeśli nie ma użytkownika, ustaw loading na false (zostanie przekierowany przez ProtectedRoute)
        setLoading(false);
      }
    };
  ...
  const loadProposal = async () => {
    ...
    try {
      ...
      if (linkData) {
        const publicUrl = `${window.location.origin}/p/${linkData.token}`;
